...
- `expo-debug.log`: Captures Expo/Metro stdout and stderr. Generated by running `npm run debug:expo` (starts at port 8082 and moves to the next available port).
...
3. **Reproduce the break**: launch the app (e.g. from another terminal run `npx expo start --port 8082` or another available port, then open the app on the device/emulator) and trigger the crash or bad behavior.
