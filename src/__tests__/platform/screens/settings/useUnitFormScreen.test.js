/**
 * useUnitFormScreen Hook Tests
 */
const { renderHook, act } = require('@testing-library/react-native');
const useUnitFormScreen = require('@platform/screens/settings/UnitFormScreen/useUnitFormScreen').default;

const mockPush = jest.fn();
const mockReplace = jest.fn();
let mockParams = {};

jest.mock('expo-router', () => ({
  useRouter: () => ({ push: mockPush, replace: mockReplace }),
  useLocalSearchParams: () => mockParams,
}));

jest.mock('@hooks', () => ({
  useI18n: jest.fn(() => ({ t: (k) => k })),
  useNetwork: jest.fn(() => ({ isOffline: false })),
  useUnit: jest.fn(),
  useTenant: jest.fn(),
  useFacility: jest.fn(),
  useDepartment: jest.fn(),
  useTenantAccess: jest.fn(),
}));

const {
  useI18n,
  useNetwork,
  useUnit,
  useTenant,
  useFacility,
  useDepartment,
  useTenantAccess,
} = require('@hooks');

describe('useUnitFormScreen', () => {
  const mockGet = jest.fn();
  const mockCreate = jest.fn();
  const mockUpdate = jest.fn();
  const mockReset = jest.fn();
  const mockListTenants = jest.fn();
  const mockResetTenants = jest.fn();
  const mockListFacilities = jest.fn();
  const mockResetFacilities = jest.fn();
  const mockListDepartments = jest.fn();
  const mockResetDepartments = jest.fn();

  beforeEach(() => {
    jest.clearAllMocks();
    mockParams = {};
    useI18n.mockReturnValue({ t: (k) => k });
    useNetwork.mockReturnValue({ isOffline: false });
    useTenantAccess.mockReturnValue({
      canAccessTenantSettings: true,
      canManageAllTenants: true,
      tenantId: null,
      isResolved: true,
    });
    useUnit.mockReturnValue({
      get: mockGet,
      create: mockCreate,
      update: mockUpdate,
      data: null,
      isLoading: false,
      errorCode: null,
      reset: mockReset,
    });
    useTenant.mockReturnValue({
      list: mockListTenants,
      data: { items: [] },
      isLoading: false,
      errorCode: null,
      reset: mockResetTenants,
    });
    useFacility.mockReturnValue({
      list: mockListFacilities,
      data: { items: [] },
      isLoading: false,
      errorCode: null,
      reset: mockResetFacilities,
    });
    useDepartment.mockReturnValue({
      list: mockListDepartments,
      data: { items: [] },
      isLoading: false,
      errorCode: null,
      reset: mockResetDepartments,
    });
  });

  it('redirects users without unit access', () => {
    useTenantAccess.mockReturnValue({
      canAccessTenantSettings: false,
      canManageAllTenants: false,
      tenantId: null,
      isResolved: true,
    });

    renderHook(() => useUnitFormScreen());

    expect(mockReplace).toHaveBeenCalledWith('/settings');
  });

  it('prefills tenant for tenant-scoped admins', () => {
    useTenantAccess.mockReturnValue({
      canAccessTenantSettings: true,
      canManageAllTenants: false,
      tenantId: 'tenant-1',
      isResolved: true,
    });

    const { result } = renderHook(() => useUnitFormScreen());

    expect(result.current.tenantId).toBe('tenant-1');
    expect(result.current.tenantListLoading).toBe(false);
    expect(mockListTenants).not.toHaveBeenCalled();
  });

  it('returns initial state for create', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.isEdit).toBe(false);
    expect(result.current.name).toBe('');
    expect(result.current.isActive).toBe(true);
    expect(result.current.tenantId).toBe('');
    expect(result.current.facilityId).toBe('');
    expect(result.current.departmentId).toBe('');
    expect(result.current.unit).toBeNull();
  });

  it('calls get on mount when editing', () => {
    mockParams = { id: 'uid-1' };
    renderHook(() => useUnitFormScreen());
    expect(mockReset).toHaveBeenCalled();
    expect(mockGet).toHaveBeenCalledWith('uid-1');
  });

  it('lists tenants on mount when creating', () => {
    renderHook(() => useUnitFormScreen());
    expect(mockResetTenants).toHaveBeenCalled();
    expect(mockListTenants).toHaveBeenCalledWith({ page: 1, limit: 100 });
  });

  it('hydrates form state from unit data', () => {
    mockParams = { id: 'uid-1' };
    useUnit.mockReturnValue({
      get: mockGet,
      create: mockCreate,
      update: mockUpdate,
      data: {
        name: 'Main',
        is_active: false,
        tenant_id: 't1',
        facility_id: 'f1',
        department_id: 'd1',
      },
      isLoading: false,
      errorCode: null,
      reset: mockReset,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.name).toBe('Main');
    expect(result.current.isActive).toBe(false);
    expect(result.current.tenantId).toBe('t1');
    expect(result.current.facilityId).toBe('f1');
    expect(result.current.departmentId).toBe('d1');
  });

  it('lists facilities when tenantId is set', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
    });
    expect(mockResetFacilities).toHaveBeenCalled();
    expect(mockListFacilities).toHaveBeenCalledWith({ page: 1, limit: 100, tenant_id: 't1' });
  });

  it('lists departments when tenantId and facilityId are set', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
      result.current.setFacilityId('f1');
    });
    expect(mockResetDepartments).toHaveBeenCalled();
    expect(mockListDepartments).toHaveBeenCalledWith({
      page: 1,
      limit: 100,
      tenant_id: 't1',
      facility_id: 'f1',
    });
  });

  it('uses fallback error message for unknown error codes', () => {
    useI18n.mockReturnValue({
      t: (k) => (k === 'unit.form.submitErrorMessage' ? 'Fallback message' : k),
    });
    useUnit.mockReturnValue({
      get: mockGet,
      create: mockCreate,
      update: mockUpdate,
      data: null,
      isLoading: false,
      errorCode: 'UNKNOWN_ERROR',
      reset: mockReset,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.hasError).toBe(true);
    expect(result.current.errorMessage).toBe('Fallback message');
  });

  it('uses fallback tenant error message for unknown error codes', () => {
    useI18n.mockReturnValue({
      t: (k) => (k === 'unit.form.tenantLoadErrorMessage' ? 'Tenant fallback' : k),
    });
    useTenant.mockReturnValue({
      list: mockListTenants,
      data: { items: [] },
      isLoading: false,
      errorCode: 'UNKNOWN_ERROR',
      reset: mockResetTenants,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.tenantListError).toBe(true);
    expect(result.current.tenantErrorMessage).toBe('Tenant fallback');
  });

  it('uses fallback facility error message for unknown error codes', () => {
    useI18n.mockReturnValue({
      t: (k) => (k === 'unit.form.facilityLoadErrorMessage' ? 'Facility fallback' : k),
    });
    useFacility.mockReturnValue({
      list: mockListFacilities,
      data: { items: [] },
      isLoading: false,
      errorCode: 'UNKNOWN_ERROR',
      reset: mockResetFacilities,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.facilityListError).toBe(true);
    expect(result.current.facilityErrorMessage).toBe('Facility fallback');
  });

  it('uses fallback department error message for unknown error codes', () => {
    useI18n.mockReturnValue({
      t: (k) => (k === 'unit.form.departmentLoadErrorMessage' ? 'Department fallback' : k),
    });
    useDepartment.mockReturnValue({
      list: mockListDepartments,
      data: { items: [] },
      isLoading: false,
      errorCode: 'UNKNOWN_ERROR',
      reset: mockResetDepartments,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    expect(result.current.departmentListError).toBe(true);
    expect(result.current.departmentErrorMessage).toBe('Department fallback');
  });

  it('submits create payload and navigates on success', async () => {
    mockCreate.mockResolvedValue({ id: 'u1' });
    useTenant.mockReturnValue({
      list: mockListTenants,
      data: { items: [{ id: 't1', name: 'Tenant 1' }] },
      isLoading: false,
      errorCode: null,
      reset: mockResetTenants,
    });
    useFacility.mockReturnValue({
      list: mockListFacilities,
      data: { items: [{ id: 'f1', name: 'Facility 1' }] },
      isLoading: false,
      errorCode: null,
      reset: mockResetFacilities,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId(' t1 ');
      result.current.setFacilityId(' f1 ');
      result.current.setDepartmentId(' d1 ');
      result.current.setName('  Unit  ');
      result.current.setIsActive(false);
    });
    await act(async () => {
      await result.current.onSubmit();
    });
    expect(mockCreate).toHaveBeenCalledWith({
      tenant_id: 't1',
      name: 'Unit',
      is_active: false,
      facility_id: 'f1',
      department_id: 'd1',
    });
    expect(mockReplace).toHaveBeenCalledWith('/settings/units?notice=created');
  });

  it('submits update payload and navigates on success', async () => {
    mockParams = { id: 'uid-1' };
    mockUpdate.mockResolvedValue({ id: 'uid-1' });
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setName('  Main  ');
      result.current.setFacilityId('');
      result.current.setIsActive(true);
    });
    await act(async () => {
      await result.current.onSubmit();
    });
    expect(mockUpdate).toHaveBeenCalledWith('uid-1', {
      name: 'Main',
      is_active: true,
      facility_id: null,
      department_id: null,
    });
    expect(mockReplace).toHaveBeenCalledWith('/settings/units?notice=updated');
  });

  it('uses queued notice when offline', async () => {
    useNetwork.mockReturnValue({ isOffline: true });
    mockCreate.mockResolvedValue({ id: 'u1' });
    useTenant.mockReturnValue({
      list: mockListTenants,
      data: { items: [{ id: 't1', name: 'Tenant 1' }] },
      isLoading: false,
      errorCode: null,
      reset: mockResetTenants,
    });
    useFacility.mockReturnValue({
      list: mockListFacilities,
      data: { items: [{ id: 'f1', name: 'Facility 1' }] },
      isLoading: false,
      errorCode: null,
      reset: mockResetFacilities,
    });
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
      result.current.setName('Unit');
      result.current.setFacilityId('f1');
    });
    await act(async () => {
      await result.current.onSubmit();
    });
    expect(mockReplace).toHaveBeenCalledWith('/settings/units?notice=queued');
  });

  it('does not navigate when create returns undefined', async () => {
    mockCreate.mockResolvedValue(undefined);
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
      result.current.setName('Unit');
    });
    await act(async () => {
      await result.current.onSubmit();
    });
    expect(mockReplace).not.toHaveBeenCalled();
  });

  it('does not submit when required fields are missing', async () => {
    const { result } = renderHook(() => useUnitFormScreen());
    await act(async () => {
      await result.current.onSubmit();
    });
    expect(mockCreate).not.toHaveBeenCalled();
  });

  it('onCancel navigates back to list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    result.current.onCancel();
    expect(mockPush).toHaveBeenCalledWith('/settings/units');
  });

  it('onGoToTenants navigates to tenants list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    result.current.onGoToTenants();
    expect(mockPush).toHaveBeenCalledWith('/settings/tenants');
  });

  it('onGoToFacilities navigates to facilities list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    result.current.onGoToFacilities();
    expect(mockPush).toHaveBeenCalledWith('/settings/facilities');
  });

  it('onGoToDepartments navigates to departments list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    result.current.onGoToDepartments();
    expect(mockPush).toHaveBeenCalledWith('/settings/departments');
  });

  it('onRetryTenants reloads tenant list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    result.current.onRetryTenants();
    expect(mockResetTenants).toHaveBeenCalled();
    expect(mockListTenants).toHaveBeenCalledWith({ page: 1, limit: 100 });
  });

  it('onRetryFacilities reloads facility list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
    });
    mockResetFacilities.mockClear();
    mockListFacilities.mockClear();
    result.current.onRetryFacilities();
    expect(mockResetFacilities).toHaveBeenCalled();
    expect(mockListFacilities).toHaveBeenCalledWith({ page: 1, limit: 100, tenant_id: 't1' });
  });

  it('onRetryDepartments reloads department list', () => {
    const { result } = renderHook(() => useUnitFormScreen());
    act(() => {
      result.current.setTenantId('t1');
      result.current.setFacilityId('f1');
    });
    mockResetDepartments.mockClear();
    mockListDepartments.mockClear();
    result.current.onRetryDepartments();
    expect(mockResetDepartments).toHaveBeenCalled();
    expect(mockListDepartments).toHaveBeenCalledWith({
      page: 1,
      limit: 100,
      tenant_id: 't1',
      facility_id: 'f1',
    });
  });
});
