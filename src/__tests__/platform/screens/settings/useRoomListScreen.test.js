...
describe('useRoomListScreen', () => {
  ...
  it('blocks edit and delete when tenant-scoped user targets unknown room id', async () => {
    useTenantAccess.mockReturnValue({
      canAccessTenantSettings: true,
      canManageAllTenants: false,
      tenantId: 'tenant-1',
    ...
    act(() => {
      result.current.onEdit('room-missing');
    });
    await act(async () => {
      await result.current.onDelete('room-missing');
    });
