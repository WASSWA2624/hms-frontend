...
const PREFS_STORAGE_PREFIX = 'hms.settings.userMfa.list.preferences';
const TABLE_COLUMNS = Object.freeze(['user', 'channel', 'enabled']);
...
const FILTER_FIELDS = Object.freeze(['user', 'channel', 'enabled']);
const FILTER_LOGICS = Object.freeze(['AND', 'OR']);
const FILTER_OPERATORS = Object.freeze({
  user: ['contains', 'equals', 'startsWith'],
  channel: ['contains', 'equals', 'startsWith'],
  enabled: ['contains', 'equals'],
});

const DEFAULT_FILTER = (id = 'user-mfa-filter-1') => ({
  id,
  field: 'user',
  operator: 'contains',
  value: '',
...
const matchesUserMfaSearch = (userMfa, query, scope, resolveUserMfaFieldValue) => {
  const normalizedQuery = normalizeLower(query);
  if (!normalizedQuery) return true;

  if (scope === 'all') {
    return TABLE_COLUMNS.some((field) => normalizeLower(
      resolveUserMfaFieldValue(userMfa, field)
    ).includes(normalizedQuery));
  }

  const field = sanitizeFilterField(scope);
  return normalizeLower(resolveUserMfaFieldValue(userMfa, field)).includes(normalizedQuery);
};

const matchesUserMfaFilter = (userMfa, filter, resolveUserMfaFieldValue) => {
  const field = sanitizeFilterField(filter?.field);
  const operator = sanitizeFilterOperator(field, filter?.operator);
  const value = sanitizeFilterValue(filter?.value);
  if (!value) return true;

  const fieldValue = resolveUserMfaFieldValue(userMfa, field);
...
const compareByField = (leftUserMfa, rightUserMfa, field, direction, resolveUserMfaFieldValue) => {
  const normalizedField = sanitizeSortField(field);
  const leftValue = resolveUserMfaFieldValue(leftUserMfa, normalizedField);
  const rightValue = resolveUserMfaFieldValue(rightUserMfa, normalizedField);
...
const useUserMfaListScreen = () => {
  ...
  const [filters, setFilters] = useState([DEFAULT_FILTER('user-mfa-filter-1')]);
  ...
  const getNextFilterId = useCallback(() => {
    ...
    return `user-mfa-filter-${filterCounterRef.current}`;
  }, []);
  ...
  const resolveUserMfaTenantContext = useCallback((userMfa) => {
  ...
  const resolveUserLabel = useCallback((userMfa) => {
    const userId = normalizeValue(userMfa?.user_id);
    const userContext = userLookup.get(userId);
    ...
    if (userId) return t('userMfa.list.currentUserLabel');
    return t('common.notAvailable');
  }, [canViewTechnicalIds, t, userLookup]);

  const resolveChannelLabel = useCallback((userMfa) => {
    const channelRaw = normalizeValue(userMfa?.channel);
    if (!channelRaw) return t('common.notAvailable');
    const channelKey = `userMfa.channel.${channelRaw.toUpperCase()}`;
    const translated = t(channelKey);
    if (translated !== channelKey) return translated;
    if (canViewTechnicalIds) return channelRaw;
    return t('userMfa.list.currentChannelLabel');
  }, [canViewTechnicalIds, t]);

  const resolveEnabledLabel = useCallback((userMfa) => {
    const isEnabled = userMfa?.is_enabled;
    if (typeof isEnabled === 'boolean') {
      return isEnabled ? t('common.on') : t('common.off');
    }
    const rawValue = normalizeValue(isEnabled);
    if (!rawValue) return t('common.notAvailable');
    if (canViewTechnicalIds) return rawValue;
    return t('userMfa.list.currentStatusLabel');
  }, [canViewTechnicalIds, t]);

  const resolveUserMfaFieldValue = useCallback((userMfa, field) => {
    if (field === 'user') return resolveUserLabel(userMfa);
    if (field === 'channel') return resolveChannelLabel(userMfa);
    if (field === 'enabled') return resolveEnabledLabel(userMfa);
    return '';
  }, [resolveUserLabel, resolveChannelLabel, resolveEnabledLabel]);
  ...
  const scopedRawItems = useMemo(() => {
    ...
    return rawItems.filter((userMfa) => {
      const { tenantIds } = resolveUserMfaTenantContext(userMfa);
  ...
  const filteredItems = useMemo(() => {
    ...
    return scopedRawItems.filter((userMfa) => {
      if (hasSearch && !matchesUserMfaSearch(
        userMfa,
        normalizedSearch,
        normalizedSearchScope,
        resolveUserMfaFieldValue
      )) {
        return false;
      }
      ...
      const matches = activeFilters.map((filter) => matchesUserMfaFilter(
        userMfa,
        filter,
        resolveUserMfaFieldValue
      ));
  ...
  ), [filteredItems, sortField, sortDirection, resolveUserMfaFieldValue]);
  ...
  const canAccessUserMfaRecord = useCallback((userMfa) => {
    ...
    if (tenantIds.length === 0) return false;
    return tenantIds.every((tenantIdValue) => tenantIdValue === normalizedTenantId);
  }, [canManageAllTenants, normalizedTenantId, resolveUserMfaTenantContext]);

  const handleItemPress = useCallback((id) => {
    if (!canManageUserMfas) return;
    const userMfaId = normalizeValue(id);
    if (!userMfaId) return;

    const targetUserMfa = resolveUserMfaById(userMfaId);
    if (!targetUserMfa && !canManageAllTenants) {
      router.push('/settings/user-mfas?notice=accessDenied');
      return;
    }
    if (targetUserMfa && !canAccessUserMfaRecord(targetUserMfa)) {
      router.push('/settings/user-mfas?notice=accessDenied');
      return;
    }

    router.push(`/settings/user-mfas/${userMfaId}`);
  }, [
    canManageUserMfas,
    resolveUserMfaById,
    canManageAllTenants,
    canAccessUserMfaRecord,
    router,
  ]);
  ...
  const searchScopeOptions = useMemo(() => ([
    { value: 'all', label: t('userMfa.list.searchScopeAll') },
    { value: 'user', label: t('userMfa.list.searchScopeUser') },
    { value: 'channel', label: t('userMfa.list.searchScopeChannel') },
    { value: 'enabled', label: t('userMfa.list.searchScopeEnabled') },
  ]), [t]);

  const filterFieldOptions = useMemo(() => ([
    { value: 'user', label: t('userMfa.list.filterFieldUser') },
    { value: 'channel', label: t('userMfa.list.filterFieldChannel') },
    { value: 'enabled', label: t('userMfa.list.filterFieldEnabled') },
  ]), [t]);

  const filterLogicOptions = useMemo(() => ([
    { value: 'AND', label: t('userMfa.list.filterLogicAnd') },
    { value: 'OR', label: t('userMfa.list.filterLogicOr') },
  ]), [t]);
  ...
  const densityOptions = useMemo(() => ([
    { value: 'compact', label: t('userMfa.list.densityCompact') },
    { value: 'comfortable', label: t('userMfa.list.densityComfortable') },
  ]), [t]);
  ...
  const resolveFilterOperatorOptions = useCallback((field) => {
    ...
    return operators.map((operator) => ({
      value: operator,
      label: t(`userMfa.list.filterOperator${operator}`),
    }));
  }, [t]);
