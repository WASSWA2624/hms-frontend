/**
 * useRolePermissionListScreen Hook
 * Shared logic for RolePermissionListScreen across platforms.
 * File: useRolePermissionListScreen.js
 */
...
const PREFS_STORAGE_PREFIX = 'hms.settings.rolePermissions.list.preferences';
const TABLE_COLUMNS = Object.freeze(['role', 'permission', 'tenant']);
...
const FILTER_FIELDS = Object.freeze(['role', 'permission', 'tenant']);
...
const DEFAULT_FILTER = (id = 'role-permission-filter-1') => ({
  id,
  field: 'role',
  operator: 'contains',
  value: '',
...
const sanitizeSortField = (value) => (TABLE_COLUMNS.includes(value) ? value : 'role');
...
const sanitizeFilterField = (value) => (
  FILTER_FIELDS.includes(value) ? value : 'role'
);
...
const resolveNoticeMessage = (t, notice) => {
  const map = {
    created: 'rolePermission.list.noticeCreated',
...
const matchesRolePermissionSearch = (rolePermission, query, scope, resolveRolePermissionFieldValue) => {
  const normalizedQuery = normalizeLower(query);
  if (!normalizedQuery) return true;
  if (scope === 'all') {
    return TABLE_COLUMNS.some((field) => normalizeLower(
      resolveRolePermissionFieldValue(rolePermission, field)
    ).includes(normalizedQuery));
  }

  const field = sanitizeFilterField(scope);
  return normalizeLower(resolveRolePermissionFieldValue(rolePermission, field)).includes(normalizedQuery);
};

const matchesRolePermissionFilter = (rolePermission, filter, resolveRolePermissionFieldValue) => {
  const field = sanitizeFilterField(filter?.field);
  const operator = sanitizeFilterOperator(field, filter?.operator);
  const value = sanitizeFilterValue(filter?.value);
  if (!value) return true;

  const fieldValue = resolveRolePermissionFieldValue(rolePermission, field);
...
const compareByField = (
  ...
  field,
  direction,
  resolveRolePermissionFieldValue
) => {
  const normalizedField = sanitizeSortField(field);
  const leftValue = resolveRolePermissionFieldValue(leftRolePermission, normalizedField);
  const rightValue = resolveRolePermissionFieldValue(rightRolePermission, normalizedField);
  const result = compareText(leftValue, rightValue);
  return direction === 'desc' ? result * -1 : result;
};

const useRolePermissionListScreen = () => {
  const { t } = useI18n();
  const { user } = useAuth();
  ...
  const [filters, setFilters] = useState([DEFAULT_FILTER('role-permission-filter-1')]);
  ...
  const getNextFilterId = useCallback(() => {
    ...
    return `role-permission-filter-${filterCounterRef.current}`;
  }, []);
  ...
  const roleLookup = useMemo(() => {
    ...
    roleItems.forEach((role, index) => {
      ...
      const roleLabel = resolveReadableValue(role?.name, role?.slug)
        || (canViewTechnicalIds ? roleId : t('rolePermission.form.roleOptionFallback', { index: index + 1 }));
      if (roleLabel) {
  ...
  const permissionLookup = useMemo(() => {
    ...
    permissionItems.forEach((permission, index) => {
      ...
      if (permissionLabel) {
        map.set(permissionId, permissionLabel);
      }
    });
    return map;
  }, [permissionItems, canViewTechnicalIds, t]);

  const resolveRoleLabel = useCallback((rolePermission) => {
    const roleId = normalizeValue(rolePermission?.role_id);
    ...
    if (roleId) return t('rolePermission.list.currentRoleLabel');
    return t('common.notAvailable');
  }, [roleLookup, canViewTechnicalIds, t]);

  const resolvePermissionLabel = useCallback((rolePermission) => {
    const permissionId = normalizeValue(rolePermission?.permission_id);
    const readablePermission = resolveReadableValue(
      rolePermission?.permission_name,
      rolePermission?.permission?.name,
      rolePermission?.permission_label,
      permissionLookup.get(permissionId)
    );
    if (readablePermission) return readablePermission;
    if (permissionId && canViewTechnicalIds) return permissionId;
    if (permissionId) return t('rolePermission.list.currentPermissionLabel');
    return t('common.notAvailable');
  }, [permissionLookup, canViewTechnicalIds, t]);

  const resolveTenantLabel = useCallback((rolePermission) => {
    ...
    if (rolePermissionTenantId && canViewTechnicalIds) return rolePermissionTenantId;
    if (rolePermissionTenantId) return t('rolePermission.list.currentTenantLabel');
    return t('common.notAvailable');
  }, [canViewTechnicalIds, t]);

  const resolveRolePermissionFieldValue = useCallback((rolePermission, field) => {
    if (field === 'role') return resolveRoleLabel(rolePermission);
    if (field === 'permission') return resolvePermissionLabel(rolePermission);
    if (field === 'tenant') return resolveTenantLabel(rolePermission);
    return '';
  }, [resolveRoleLabel, resolvePermissionLabel, resolveTenantLabel]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return rawItems.filter((rolePermission) => {
      ...
      const matches = activeFilters.map((filter) => matchesRolePermissionFilter(
        rolePermission,
        filter,
        resolveRolePermissionFieldValue
      ));
  ...
  ), [filteredItems, sortField, sortDirection, resolveRolePermissionFieldValue]);
  ...
  const handleItemPress = useCallback((id) => {
    ...
    const rolePermissionId = normalizeValue(id);
  ...
  const searchScopeOptions = useMemo(() => ([
    { value: 'all', label: t('rolePermission.list.searchScopeAll') },
    { value: 'role', label: t('rolePermission.list.searchScopeRole') },
    { value: 'permission', label: t('rolePermission.list.searchScopePermission') },
    { value: 'tenant', label: t('rolePermission.list.searchScopeTenant') },
  ]), [t]);

  const filterFieldOptions = useMemo(() => ([
    { value: 'role', label: t('rolePermission.list.filterFieldRole') },
    { value: 'permission', label: t('rolePermission.list.filterFieldPermission') },
    { value: 'tenant', label: t('rolePermission.list.filterFieldTenant') },
  ]), [t]);

  const filterLogicOptions = useMemo(() => ([
    { value: 'AND', label: t('rolePermission.list.filterLogicAnd') },
    { value: 'OR', label: t('rolePermission.list.filterLogicOr') },
  ]), [t]);
  ...
  const densityOptions = useMemo(() => ([
    { value: 'compact', label: t('rolePermission.list.densityCompact') },
    { value: 'comfortable', label: t('rolePermission.list.densityComfortable') },
  ]), [t]);
  ...
  const visibleOrderedColumns = useMemo(() => {
    ...
    if (filtered.length === 0) return ['role'];
    return filtered;
  ...
  const resolveFilterOperatorOptions = useCallback((field) => {
    const normalizedField = sanitizeFilterField(field);
    const operators = FILTER_OPERATORS[normalizedField] || FILTER_OPERATORS.role;
    return operators.map((operator) => ({
      value: operator,
      label: t(`rolePermission.list.filterOperator${operator}`),
    }));
  }, [t]);
...
export default useRolePermissionListScreen;
