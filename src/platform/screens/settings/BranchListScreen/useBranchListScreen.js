...
import { useAuth, useI18n, useNetwork, useBranch, useTenantAccess } from '@hooks';
import { async as asyncStorage } from '@services/storage';
import { confirmAction, humanizeIdentifier } from '@utils';

const TABLE_MODE_BREAKPOINT = 768;
const PREFS_STORAGE_PREFIX = 'hms.settings.branches.list.preferences';
const BRANCH_CACHE_STORAGE_PREFIX = 'hms.settings.branches.list.cache';
...
const DEFAULT_FILTER = (id = 'branch-filter-1') => ({
  id,
  field: 'name',
  operator: 'contains',
  value: '',
...
const resolveBranchName = (branch) => normalizeValue(humanizeIdentifier(branch?.name));
const resolveBranchTenant = (branch) => normalizeValue(
  humanizeIdentifier(
    branch?.tenant_name
    ?? branch?.tenant?.name
    ?? branch?.tenant_label
...
const resolveBranchFieldValue = (branch, field) => {
  if (field === 'name') return resolveBranchName(branch);
  if (field === 'tenant') return resolveBranchTenant(branch);
  if (field === 'facility') return resolveBranchFacility(branch);
  if (field === 'status') return resolveBranchStatus(branch);
  return '';
};
...
const matchesBranchSearch = (branch, query, scope) => {
  const normalizedQuery = normalizeLower(query);
  if (!normalizedQuery) return true;
  if (scope === 'all') {
    return TABLE_COLUMNS.some((field) => normalizeLower(resolveBranchFieldValue(branch, field))
      .includes(normalizedQuery));
  }
  const field = sanitizeFilterField(scope);
  return normalizeLower(resolveBranchFieldValue(branch, field)).includes(normalizedQuery);
};

const matchesBranchFilter = (branch, filter) => {
  const field = sanitizeFilterField(filter?.field);
  const operator = sanitizeFilterOperator(field, filter?.operator);
  const value = sanitizeFilterValue(filter?.value);
  if (!value) return true;

  const fieldValue = resolveBranchFieldValue(branch, field);
...
const compareByField = (leftBranch, rightBranch, field, direction) => {
  const normalizedField = sanitizeSortField(field);
  const leftValue = resolveBranchFieldValue(leftBranch, normalizedField);
  const rightValue = resolveBranchFieldValue(rightBranch, normalizedField);
...
const useBranchListScreen = () => {
  ...
  const [filters, setFilters] = useState([DEFAULT_FILTER('branch-filter-1')]);
  ...
  const [cachedBranchItems, setCachedBranchItems] = useState([]);
  ...
  const branchDataCacheKey = useMemo(() => {
    const scope = canManageAllTenants
      ? 'all'
      : (normalizedTenantId || 'self');
    return `${BRANCH_CACHE_STORAGE_PREFIX}.${preferenceSubject}.${scope}`;
  ...
  const getNextFilterId = useCallback(() => {
    ...
    return `branch-filter-${filterCounterRef.current}`;
  }, []);
  ...
  const hasLiveBranchPayload = useMemo(
  ...
  const baseItems = useMemo(() => {
    ...
    if (isOffline && cachedBranchItems.length > 0) return cachedBranchItems;
    return liveItems;
  ...
  const filteredItems = useMemo(() => {
    ...
    return scopedItems.filter((branch) => {
      ...
      const matches = activeFilters.map((filter) => matchesBranchFilter(branch, filter));
      if (filterLogic === 'OR') {
  ...
  const currentPageBranchIds = useMemo(
    () => pagedItems.map((branch) => branch?.id).filter(Boolean),
    [pagedItems]
  );

  const selectedOnPageCount = useMemo(
    () => currentPageBranchIds.filter((id) => selectedBranchIds.includes(id)).length,
    [currentPageBranchIds, selectedBranchIds]
  );

  const allPageSelected = currentPageBranchIds.length > 0
  ...
  useEffect(() => {
    ...
    const loadCachedBranchItems = async () => {
      const stored = await asyncStorage.getItem(branchDataCacheKey);
      if (cancelled) return;

      if (Array.isArray(stored)) {
        setCachedBranchItems(stored);
        return;
      }

      if (Array.isArray(stored?.items)) {
        setCachedBranchItems(stored.items);
        return;
      }

      setCachedBranchItems([]);
    };

    loadCachedBranchItems();
  ...
  useEffect(() => {
    if (!isResolved || !canManageBranches || !hasLiveBranchPayload) return;
    setCachedBranchItems(liveItems);
    asyncStorage.setItem(branchDataCacheKey, liveItems);
  }, [
    isResolved,
    canManageBranches,
    hasLiveBranchPayload,
    branchDataCacheKey,
    liveItems,
  ]);
  ...
  const handleToggleBranchSelection = useCallback((branchIdValue) => {
    const normalizedId = normalizeValue(branchIdValue);
  ...
  const handleTogglePageSelection = useCallback((checked) => {
    setSelectedBranchIds((previous) => {
      if (!checked) {
        const pageIdSet = new Set(currentPageBranchIds);
        return previous.filter((value) => !pageIdSet.has(value));
      }
      const merged = new Set([...previous, ...currentPageBranchIds]);
      return [...merged];
    });
  }, [currentPageBranchIds]);
  ...
  const resolveBranchById = useCallback((branchIdValue) => (
    items.find((branch) => branch?.id === branchIdValue) ?? null
  ), [items]);

  const canAccessBranchRecord = useCallback((branch) => {
    if (!branch) return false;
    if (canManageAllTenants) return true;

    const branchTenantId = normalizeValue(branch?.tenant_id);
    if (!branchTenantId || !normalizedTenantId) return true;
  ...
  const handleDelete = useCallback(
    async (branchIdValue, e) => {
      ...
      try {
        ...
        if (!result) return;
        fetchList();
        setSelectedBranchIds((previous) => previous.filter((value) => value !== normalizedId));
  ...
  const handleBulkDelete = useCallback(async () => {
    if (!canDeleteBranch || selectedBranchIds.length === 0) return;
    if (!confirmAction(t('branch.list.bulkDeleteConfirm', { count: selectedBranchIds.length }))) {
      return;
    }
    ...
    for (const branchIdValue of selectedBranchIds) {
      ...
      try {
        const result = await remove(branchIdValue);
  ...
  useEffect(() => {
    const availableIds = new Set(items.map((branch) => branch?.id).filter(Boolean));
    setSelectedBranchIds((previous) => previous.filter((value) => availableIds.has(value)));
  }, [items]);
  ...
  const searchScopeOptions = useMemo(() => ([
    { value: 'all', label: t('branch.list.searchScopeAll') },
    { value: 'name', label: t('branch.list.searchScopeName') },
    { value: 'tenant', label: t('branch.list.searchScopeTenant') },
    { value: 'facility', label: t('branch.list.searchScopeFacility') },
    { value: 'status', label: t('branch.list.searchScopeStatus') },
  ]), [t]);

  const filterFieldOptions = useMemo(() => ([
    { value: 'name', label: t('branch.list.filterFieldName') },
    { value: 'tenant', label: t('branch.list.filterFieldTenant') },
    { value: 'facility', label: t('branch.list.filterFieldFacility') },
    { value: 'status', label: t('branch.list.filterFieldStatus') },
  ]), [t]);

  const filterLogicOptions = useMemo(() => ([
    { value: 'AND', label: t('branch.list.filterLogicAnd') },
    { value: 'OR', label: t('branch.list.filterLogicOr') },
  ]), [t]);
  ...
  const densityOptions = useMemo(() => ([
    { value: 'compact', label: t('branch.list.densityCompact') },
    { value: 'comfortable', label: t('branch.list.densityComfortable') },
  ]), [t]);
  ...
  const resolveFilterOperatorOptions = useCallback((field) => {
    ...
    return operators.map((operator) => ({
      value: operator,
      label: t(`branch.list.filterOperator${operator}`),
    }));
  }, [t]);
  ...
  return {
    ...
    onBulkDelete: canDeleteBranch ? handleBulkDelete : undefined,
