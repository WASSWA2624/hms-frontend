...
import { useAuth, useI18n, useNetwork, useUnit, useTenantAccess } from '@hooks';
import { async as asyncStorage } from '@services/storage';
import { confirmAction, humanizeIdentifier } from '@utils';

const TABLE_MODE_BREAKPOINT = 768;
const PREFS_STORAGE_PREFIX = 'hms.settings.units.list.preferences';
const UNIT_CACHE_STORAGE_PREFIX = 'hms.settings.units.list.cache';
const TABLE_COLUMNS = Object.freeze(['name', 'tenant', 'facility', 'department', 'status']);
...
const FILTER_FIELDS = Object.freeze(['name', 'tenant', 'facility', 'department', 'status']);
...
const FILTER_OPERATORS = Object.freeze({
  ...
  department: ['contains', 'equals', 'startsWith'],
  status: ['is'],
});

const DEFAULT_FILTER = (id = 'unit-filter-1') => ({
  id,
  field: 'name',
  operator: 'contains',
  value: '',
...
const resolveUnitName = (Unit) => normalizeValue(humanizeIdentifier(Unit?.name));
const resolveUnitTenant = (Unit) => normalizeValue(
  humanizeIdentifier(
    Unit?.tenant_name
    ?? Unit?.tenant?.name
    ?? Unit?.tenant_label
...
const resolveUnitFacility = (Unit) => normalizeValue(
  humanizeIdentifier(
    Unit?.facility_name
    ?? Unit?.facility?.name
    ?? Unit?.facility_label
...
const resolveUnitDepartment = (Unit) => normalizeValue(
  humanizeIdentifier(
...
const resolveUnitStatus = (Unit) => (Unit?.is_active ? 'active' : 'inactive');

const resolveUnitFieldValue = (Unit, field) => {
  if (field === 'name') return resolveUnitName(Unit);
  if (field === 'tenant') return resolveUnitTenant(Unit);
  if (field === 'facility') return resolveUnitFacility(Unit);
  if (field === 'department') return resolveUnitDepartment(Unit);
  if (field === 'status') return resolveUnitStatus(Unit);
  return '';
};
...
const matchesUnitSearch = (Unit, query, scope) => {
  const normalizedQuery = normalizeLower(query);
  if (!normalizedQuery) return true;
  if (scope === 'all') {
    return TABLE_COLUMNS.some((field) => normalizeLower(resolveUnitFieldValue(Unit, field))
      .includes(normalizedQuery));
  }
  const field = sanitizeFilterField(scope);
  return normalizeLower(resolveUnitFieldValue(Unit, field)).includes(normalizedQuery);
};

const matchesUnitFilter = (Unit, filter) => {
  const field = sanitizeFilterField(filter?.field);
  const operator = sanitizeFilterOperator(field, filter?.operator);
  const value = sanitizeFilterValue(filter?.value);
  if (!value) return true;

  const fieldValue = resolveUnitFieldValue(Unit, field);
...
const compareByField = (leftUnit, rightUnit, field, direction) => {
  const normalizedField = sanitizeSortField(field);
  const leftValue = resolveUnitFieldValue(leftUnit, normalizedField);
  const rightValue = resolveUnitFieldValue(rightUnit, normalizedField);
...
const useUnitListScreen = () => {
  ...
  const [filters, setFilters] = useState([DEFAULT_FILTER('unit-filter-1')]);
  ...
  const [cachedUnitItems, setCachedUnitItems] = useState([]);
  ...
  const unitDataCacheKey = useMemo(() => {
    const scope = canManageAllTenants
      ? 'all'
      : (normalizedTenantId || 'self');
    return `${UNIT_CACHE_STORAGE_PREFIX}.${preferenceSubject}.${scope}`;
  ...
  const getNextFilterId = useCallback(() => {
    ...
    return `unit-filter-${filterCounterRef.current}`;
  }, []);
  ...
  const hasLiveUnitPayload = useMemo(
  ...
  const baseItems = useMemo(() => {
    ...
    if (isOffline && cachedUnitItems.length > 0) return cachedUnitItems;
    return liveItems;
  ...
  const currentPageUnitIds = useMemo(
    () => pagedItems.map((Unit) => Unit?.id).filter(Boolean),
    [pagedItems]
  );

  const selectedOnPageCount = useMemo(
    () => currentPageUnitIds.filter((id) => selectedUnitIds.includes(id)).length,
    [currentPageUnitIds, selectedUnitIds]
  );

  const allPageSelected = currentPageUnitIds.length > 0
  ...
  useEffect(() => {
    ...
    const loadCachedUnitItems = async () => {
      const stored = await asyncStorage.getItem(unitDataCacheKey);
      if (cancelled) return;

      if (Array.isArray(stored)) {
        setCachedUnitItems(stored);
        return;
      }

      if (Array.isArray(stored?.items)) {
        setCachedUnitItems(stored.items);
        return;
      }
  ...
  useEffect(() => {
    if (!isResolved || !canManageUnits || !hasLiveUnitPayload) return;
    setCachedUnitItems(liveItems);
    asyncStorage.setItem(unitDataCacheKey, liveItems);
  }, [
    isResolved,
    canManageUnits,
    hasLiveUnitPayload,
  ...
  const handleToggleUnitSelection = useCallback((unitIdValue) => {
    const normalizedId = normalizeValue(unitIdValue);
  ...
  const handleTogglePageSelection = useCallback((checked) => {
    setSelectedUnitIds((previous) => {
      if (!checked) {
        const pageIdSet = new Set(currentPageUnitIds);
        return previous.filter((value) => !pageIdSet.has(value));
      }
      const merged = new Set([...previous, ...currentPageUnitIds]);
      return [...merged];
    });
  }, [currentPageUnitIds]);
  ...
  const resolveUnitById = useCallback((unitIdValue) => (
    items.find((Unit) => Unit?.id === unitIdValue) ?? null
  ), [items]);

  const canAccessUnitRecord = useCallback((Unit) => {
  ...
  const handleDelete = useCallback(
    async (unitIdValue, e) => {
      ...
      try {
        ...
        if (!result) return;
        fetchList();
        setSelectedUnitIds((previous) => previous.filter((value) => value !== normalizedId));
  ...
  const handleBulkDelete = useCallback(async () => {
    if (!canDeleteUnit || selectedUnitIds.length === 0) return;
    if (!confirmAction(t('unit.list.bulkDeleteConfirm', { count: selectedUnitIds.length }))) {
      return;
    }
    ...
    for (const unitIdValue of selectedUnitIds) {
      ...
      try {
        const result = await remove(unitIdValue);
  ...
  useEffect(() => {
    const availableIds = new Set(items.map((Unit) => Unit?.id).filter(Boolean));
    setSelectedUnitIds((previous) => previous.filter((value) => availableIds.has(value)));
  }, [items]);
  ...
  const searchScopeOptions = useMemo(() => ([
    { value: 'all', label: t('unit.list.searchScopeAll') },
    { value: 'name', label: t('unit.list.searchScopeName') },
    { value: 'tenant', label: t('unit.list.searchScopeTenant') },
    { value: 'facility', label: t('unit.list.searchScopeFacility') },
    { value: 'department', label: t('unit.list.searchScopeDepartment') },
    { value: 'status', label: t('unit.list.searchScopeStatus') },
  ]), [t]);

  const filterFieldOptions = useMemo(() => ([
    { value: 'name', label: t('unit.list.filterFieldName') },
    { value: 'tenant', label: t('unit.list.filterFieldTenant') },
    { value: 'facility', label: t('unit.list.filterFieldFacility') },
    { value: 'department', label: t('unit.list.filterFieldDepartment') },
    { value: 'status', label: t('unit.list.filterFieldStatus') },
  ]), [t]);

  const filterLogicOptions = useMemo(() => ([
    { value: 'AND', label: t('unit.list.filterLogicAnd') },
    { value: 'OR', label: t('unit.list.filterLogicOr') },
  ]), [t]);
  ...
  const densityOptions = useMemo(() => ([
    { value: 'compact', label: t('unit.list.densityCompact') },
    { value: 'comfortable', label: t('unit.list.densityComfortable') },
  ]), [t]);
  ...
  const resolveFilterOperatorOptions = useCallback((field) => {
    ...
    return operators.map((operator) => ({
      value: operator,
      label: t(`unit.list.filterOperator${operator}`),
    }));
  }, [t]);
  ...
  return {
    ...
    onBulkDelete: canDeleteUnit ? handleBulkDelete : undefined,
