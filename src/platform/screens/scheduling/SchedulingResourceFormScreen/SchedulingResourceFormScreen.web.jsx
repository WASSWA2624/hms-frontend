...
import {
  ...
  StyledRepeaterItem,
  StyledRepeaterStack,
} from './SchedulingResourceFormScreen.web.styles';
import useSchedulingResourceFormScreen from './useSchedulingResourceFormScreen';

const SchedulingResourceFormScreenWeb = ({ resourceId }) => {
  const { t } = useI18n();
  const {
    config,
    isEdit,
  ...
  if (isEdit && !record && isLoading) {
    return (
      <StyledContainer role="main" aria-label={t(`${config.i18nKey}.form.editTitle`)}>
        <StyledContent>
  ...
  return (
    <StyledContainer
      role="main"
      aria-label={isEdit ? t(`${config.i18nKey}.form.editTitle`) : t(`${config.i18nKey}.form.createTitle`)}
    >
      <StyledContent>
              ...
              <StyledFullRow>
                <StyledFieldGroup>
                  <TextField
                    ...
                    onChange={(event) => setFieldValue('tenant_id', event.target.value)}
                  ...
                  <StyledFieldGroup key={field.name}>
                    <StyledLookupStack>
                      <TextField
                        ...
                        onChange={(event) => setLookupQuery(field.name, event.target.value)}
                                ...
                                return (
                                  <StyledFieldGroup key={`${field.name}-${index}-${itemField.name}`}>
                                    <TextField
                                      ...
                                      value={itemValue || ''}
                                      onChange={(event) =>
                                        setRepeaterFieldValue(field.name, index, itemField.name, event.target.value)
                                      }
                                      type={itemField.type === 'datetime' ? 'datetime-local' : undefined}
                                      accessibilityLabel={t(itemField.labelKey)}
                                      accessibilityHint={t(itemField.hintKey)}
...
export default SchedulingResourceFormScreenWeb;
