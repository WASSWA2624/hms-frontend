...
const StyledContainer = styled.section`
  ...
  background:
    linear-gradient(162deg, rgba(18, 120, 92, 0.1) 0%, rgba(18, 120, 92, 0) 74%),
...
const StyledPillButton = styled(Pressable)`
  border: 1px solid ${({ $active, theme }) => ($active ? '#0f766e' : theme.colors.border.light)};
  background: ${({ $active, theme }) => ($active ? '#e6f9f6' : theme.colors.surface.primary)};
  border-radius: ${({ theme }) => theme.radius.pill || 999}px;
...
const StyledPillText = styled.span`
  ...
  color: ${({ $active, theme }) => ($active ? '#0f766e' : theme.colors.text.secondary)};
`;
...
const StyledWorklistItem = styled(Pressable)`
  ...
  flex-direction: column;
  gap: ${({ theme }) => theme.spacing.xs}px;
  border: 1px solid ${({ $selected, theme }) => ($selected ? '#0f766e' : theme.colors.border.light)};
  border-left: 4px solid ${({ $selected, theme }) => ($selected ? '#1f9d72' : theme.colors.border.medium)};
  border-radius: ${({ theme }) => theme.radius.md + 2}px;
  padding: ${({ theme }) => `${theme.spacing.xs + 2}px ${theme.spacing.sm}px`};
  background: ${({ theme, $selected }) => ($selected ? '#ecfdf7' : theme.colors.surface.primary)};
  box-shadow: ${({ $selected }) =>
    $selected ? '0 6px 14px rgba(15, 118, 110, 0.2)' : '0 2px 8px rgba(15, 23, 42, 0.08)'};
`;
