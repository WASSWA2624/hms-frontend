...
const IpdWorkbenchScreen = () => {
  ...
  return (
    <StyledContainer testID="ipd-workbench-screen">
      ...
      {screen.isLoading ? (
        <LoadingSpinner accessibilityLabel={t('common.loading')} testID="ipd-workbench-loading" />
      ) : null}

      {!screen.isLoading && !screen.canViewWorkbench ? (
        <ErrorState
          size={ErrorStateSizes.SMALL}
          title={t('ipd.workbench.states.accessDeniedTitle')}
          description={t('ipd.workbench.states.accessDeniedDescription')}
          testID="ipd-workbench-access-denied"
        />
      ) : null}
      ...
      {!screen.isLoading && screen.hasError ? (
        <ErrorState
          size={ErrorStateSizes.SMALL}
          title={t('ipd.workbench.states.loadErrorTitle')}
          description={screen.errorCode || t('ipd.workbench.states.loadErrorDescription')}
          testID="ipd-workbench-error"
        />
      ) : null}
      ...
      {!screen.isLoading ? (
        <OfflineState
          ...
          description={t('ipd.workbench.states.offlineNotice')}
          testID="ipd-workbench-offline"
          ...
          <StyledPanel>
            <Card variant="outlined">
              ...
              <StyledInlineActions>
                ...
                <Button
                  variant="surface"
                  size="small"
                  onPress={() => screen.setIsStartFormOpen((previous) => !previous)}
            ...
            {screen.isStartFormOpen ? (
              <Card variant="outlined" testID="ipd-workbench-start-form">
                ...
                <StyledFieldRow>
                  <TextField
                    label={t('ipd.workbench.start.patientId')}
                    value={screen.startDraft.patient_id}
                    onChangeText={(value) => screen.onStartDraftChange('patient_id', value)}
                    density="compact"
                  />
                  <TextField
                    label={t('ipd.workbench.start.encounterId')}
                    value={screen.startDraft.encounter_id}
                    onChangeText={(value) => screen.onStartDraftChange('encounter_id', value)}
                    density="compact"
                  />
                  <Select
                    label={t('ipd.workbench.start.initialBed')}
                    value={screen.startDraft.bed_id}
                    options={[{ value: '', label: t('common.none') }, ...bedOptions]}
                    onValueChange={(value) => screen.onStartDraftChange('bed_id', value)}
                    compact
                    searchable
                  />
                  <TextField
                    label={t('ipd.workbench.start.admittedAt')}
                    value={screen.startDraft.admitted_at}
                    onChangeText={(value) => screen.onStartDraftChange('admitted_at', value)}
                    type="datetime-local"
                    density="compact"
                  />
                </StyledFieldRow>
                <StyledInlineActions>
                  <Button
                    variant="primary"
                    size="small"
                    onPress={screen.onStartAdmission}
                    disabled={!screen.canMutate}
                    testID="ipd-workbench-start-submit"
                  >
                    {t('ipd.workbench.actions.startAdmission')}
                  </Button>
                </StyledInlineActions>
            ...
            <Card variant="outlined">
              <StyledInlineActions>
                ...
                {screen.pagination?.total ? (
                  <Text variant="caption">{t('ipd.workbench.list.total', { total: screen.pagination.total })}</Text>
                ) : null}
              </StyledInlineActions>

              {screen.flowList.length === 0 ? (
                <EmptyState
                  title={t('ipd.workbench.states.emptyTitle')}
                  description={t('ipd.workbench.states.emptyDescription')}
                  testID="ipd-workbench-empty"
                />
              ) : (
                <StyledFlowList>
                  {screen.flowList.map((item, index) => {
                    ...
                    const isSelected =
                      String(screen.selectedFlowId || '').toUpperCase() === String(flowId || '').toUpperCase();
                    return (
                      <StyledFlowListItem
                        key={flowId || `ipd-flow-${index + 1}`}
                        onPress={() => screen.onSelectFlow(item)}
                        $selected={isSelected}
                        accessibilityRole="button"
                        testID={`ipd-workbench-list-item-${index + 1}`}
                      >
                        <StyledFlowTitleRow>
                        ...
                        <StyledFlowMeta>
                          {`${t('ipd.workbench.snapshot.admissionId')}: ${flowId || t('common.notAvailable')}`}
                        </StyledFlowMeta>
                        <StyledFlowMeta>
                          {`${t('ipd.workbench.snapshot.ward')}: ${item.ward_display_name || t('common.notAvailable')}`}
                        </StyledFlowMeta>
                      </StyledFlowListItem>
                    );
                  })}
          ...
          <StyledPanel>
            <Card variant="outlined">
              ...
              {!screen.selectedFlow ? (
                ...
                <StyledSnapshotGrid>
                  ...
                  {renderSnapshotField(
                    t('ipd.workbench.snapshot.nextStep'),
                    screen.selectedSummary.nextStep,
                    t('common.notAvailable')
                  )}
                  {renderSnapshotField(
            ...
            <Card variant="outlined">
              ...
              <StyledFieldRow>
                ...
                />
                <StyledInlineActions>
                  <Button variant="primary" size="small" onPress={screen.onAssignBed} disabled={!screen.canMutate}>
                    {t('ipd.workbench.actions.assignBed')}
                  </Button>
                  <Button variant="surface" size="small" onPress={screen.onReleaseBed} disabled={!screen.canMutate}>
              ...
              <StyledFieldRow>
                ...
                <TextField
                  ...
                  onChangeText={(value) => screen.onTransferDraftChange('transfer_request_id', value)}
                  density="compact"
                />
                <StyledInlineActions>
                  <Button
